// ═══════════════════════════════════════════
// FitZone Pro — app.js
// Frontend uygulama mantığı — SPA (Single Page Application)
// API sunucusuyla iletişim, üye yönetimi, grafik, tema, oturum yönetimi
// ═══════════════════════════════════════════

// ─── XSS KORUMASI (Global Fetch Wrapper) ───
// Tüm fetch yanıtlarındaki JSON string'lerini otomatik sanitize eder
// HTML özel karakterlerini (&, <, >, ", ') güvenli karşılıklarıyla değiştirir
const originalFetch = window.fetch; // Orijinal fetch fonksiyonunu sakla
window.fetch = async (...args) => { // fetch'i XSS korumalı versiyonla değiştir
  const response = await originalFetch(...args); // Orijinal fetch'i çağır
  const contentType = response.headers.get('content-type'); // Yanıt tipini kontrol et
  if (contentType && contentType.includes('application/json')) { // Sadece JSON yanıtları filtrele
    const originalJson = response.json.bind(response); // Orijinal .json() metodunu sakla
    response.json = async () => { // .json() metodunu override et
      const json = await originalJson(); // Orijinal JSON'ı çöz
      const sanitize = (obj) => { // Rekürsif sanitize fonksiyonu
        if (typeof obj === 'string') {
          // String değerlerdeki tehlikeli HTML karakterlerini escape et
          return obj.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
        }
        if (Array.isArray(obj)) return obj.map(sanitize); // Dizi ise her elemanı sanitize et
        if (typeof obj === 'object' && obj !== null) { // Nesne ise her alanı sanitize et
          const newObj = {};
          for (const key in obj) newObj[key] = sanitize(obj[key]);
          return newObj;
        }
        return obj; // Sayı, boolean vb. olduğu gibi döndür
      };
      return sanitize(json); // Sanitize edilmiş JSON'ı döndür
    };
  }
...
// ─── DEMO VERİLER ───
// Abonelik planlarının renk ve ikon tanımları (UI kartları için)
const planColors = {
...
// Avatar arka plan renkleri — üye kartlarında kullanılır (gradient)
const avatarColors = [
...
// ─── ÜYE LİSTESİ (localStorage destekli — API yokken fallback olarak kullanılır) ───
...
// localStorage'da kayıtlı üye listesi yoksa varsayılan demo verileri kullan
let members = JSON.parse(localStorage.getItem('fitzone_members')) || [...defaultMembers];

let filtered = [...members]; // Filtrelenmiş üye listesi (arama ve durum filtresine göre)

// ─── localStorage kaydetme yardımcıları ───
function saveMembers() { localStorage.setItem('fitzone_members', JSON.stringify(members)); }       // Üye listesini kaydet
function saveRegisteredUsers() { localStorage.setItem('fitzone_users', JSON.stringify(registeredUsers)); } // Kayıtlı kullanıcıları kaydet
...
async function checkSession() {
  ...
  try {
    const csrf = localStorage.getItem('fitzone_csrf');
    // Timeout ekle — sunucu yanıt vermezse 10 saniye sonra vazgeç (sonsuz dönme önlenir)
...
function renderDerslerPage() {
  ...
  planPromise.then(() => {
    apiFetch('/api/dersler').then(data => {
      ...
      if (rTb) {
        rTb.innerHTML = ''; filteredRezervasyonlar.forEach((r, idx) => {
          const color = avatarColors[idx % avatarColors.length];
          const uyeCell = isUye ? '' : `<td><div class="member-info"><div class="m-avatar" style="background:${color};width:30px;height:30px;font-size:11px;border-radius:8px">${getInitials(r.uye)}</div><div class="m-name">${r.uye}</div></div></td>`;
